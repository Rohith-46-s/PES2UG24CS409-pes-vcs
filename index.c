...
int index_load(Index *index) {
    ...
    if (!f) return 0; // Not an error — no index yet
...
int index_save(const Index *index) {
    ...
    // Sort entries by path before writing — allocate on heap to avoid stack overflow
...
int index_add(Index *index, const char *path) {
    ...
    ObjectID oid;
    if (object_write(OBJ_BLOB, contents, (size_t)fsize, &oid) != 0) {
    ...
    uint32_t mode = S_ISDIR(st.st_mode) ? 040000 :
